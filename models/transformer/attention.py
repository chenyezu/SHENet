...
class ScaledDotProductAttentionWithHidden(nn.Module):
    ...
    def forward(self, queries, keys, values, hidden,attention_mask=None, attention_weights=None):
        ...
        hidden_k = self.fc_hidden_k(hidden).view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3) # bs h nq d_k
        hidden_v = self.fc_hidden_v(hidden).view(b_s, nq, self.h, self.d_v).permute(0, 2, 1, 3) # bs h nq d_v
