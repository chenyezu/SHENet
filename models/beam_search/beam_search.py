...
class BeamSearch(object):
    ...
    def apply(self, obj, grid, out_size=1, return_probs=False, **kwargs):
        self.b_s = utils.get_batch_size(obj)
        self.device = utils.get_device(obj)
        self.seq_mask = torch.ones((self.b_s, self.beam_size, 1), device=self.device)
        self.seq_logprob = torch.zeros((self.b_s, 1, 1), device=self.device)
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                obj, grid, outputs = self.iter(t, obj, grid, outputs, return_probs, **kwargs)
                # lp.print_stats()
    ...
    def iter(self, t: int, obj, grid, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, obj, grid, None, mode='feedback', **kwargs)
        word_logprob = word_logprob.view(self.b_s, cur_beam_size, -1)
        candidate_logprob = self.seq_logprob + word_logprob
