...
#         self.preload = preload
#         if preload:
#             self.ctx = self.load(ctx_file/"txt_ctx.hdf5",ctx_file/"vis_ctx.hdf5")
#         else:
#             self.txt_ctx = ctx_file/"txt_ctx.hdf5"
...
class GridField(RawField):
    ...
    def preprocess(self, image_id):
        key = '%d_features' % image_id
        if self.preload:
            if str(image_id) not in self.ctx:
                return torch.zeros(49, 2048)
            ctx = self.ctx[str(image_id)]
        else:
        ...
        if len(ctx) != 49:
            # 如果长度不足，填充零
            if len(ctx) < 49:
                padding = np.zeros((49 - len(ctx), 2048), dtype=np.float32)
                ctx = np.concatenate([ctx, padding], axis=0)
            # 如果长度超过，截取前49个
            else:
                ctx = ctx[:49]
