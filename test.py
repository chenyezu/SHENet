...
from data import (
    ImageDetectionsField, TextField, GridField, RawField
)
...
WATCH_IMG_IDS = {1554, 999,2473,72,36,1869}   # 改成你想看的 COCO image id, set() 则不打印
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader), dynamic_ncols=True) as pbar:
        with torch.no_grad():
            for it, data in enumerate(dataloader):
                ...
                out, _ = model(obj=obj, grid=grid, seq=captions, mode="xe")
...
def evaluate_metrics(model, dataloader, text_field, target_img_ids=None):
    model.eval()
    res={}
    gen, gts, img_ids = {}, {}, {}
    ...
    if target_img_ids is not None and target_img_ids:
        print()
        for key, img_id in img_ids.items():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='kmcn-enhanced')
    ...
    parser.add_argument('--obj_file', type=str,
                        default="./data/features/vinvl.hdf5")
    parser.add_argument('--grid_file', type=str,
                        default="./data/features/CLIP_features.hdf5")
    ...
    # Test scores
    test_scores = evaluate_metrics(model, dict_dataloader_test, text_field, target_img_ids=WATCH_IMG_IDS)
...
#             out, aux_loss = model(obj=obj, grid=grid, seq=captions, mode="xe")
#             out = out[:, :-1].contiguous()
#             captions_gt = captions[:, 1:].contiguous()
#             loss = loss_fn(out.view(-1, len(text_field.vocab)), captions_gt.view(-1))
#             loss = loss + aux_loss
...
#     parser.add_argument('--devices', nargs='+', type=int, default=[0])
#     parser.add_argument('--dataset_root', type=str,
#                         default="./data/features/ctx_features_dataset")  # ctx和全局特征的储存位置，二者将在同一个类中被读取。vis文件以img_id为keys;txt文件以{}_whole和{}_five为keys
#     parser.add_argument('--annotations', type=str,
#                         default="./annotations")  # 人工标注的GT文件
#     parser.add_argument('--obj_file', type=str,
#                         default="./data/features/vinvl.hdf5")  # 利用 Vinvl 取得的目标特征，直接以img_id作为keys
#     parser.add_argument('--grid_file', type=str,
#                         default="./data/features/CLIP_features.hdf5")  # 利用RSTNet101 取得的网格特征，以%d_features作为keys
#     parser.add_argument('--preload', action='store_true')
#     parser.add_argument('--resume_last', action='store_true')
#     parser.add_argument('--resume_best', action='store_true')
#     parser.add_argument('--fname', type=str, default='ckpt_best_test.pth')
...
#                 out = model(obj=obj, grid=grid, seq=captions, mode="xe")
#                 out = out[:, :-1].contiguous()
#                 captions_gt = captions[:, 1:].contiguous()
#                 loss = loss_fn(out.view(-1, len(text_field.vocab)), captions_gt.view(-1))
