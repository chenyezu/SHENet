...
from data import (
    ImageDetectionsField, TextField, TextFeatField, GridField, RawField
)
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader), dynamic_ncols=True) as pbar:
        with torch.no_grad():
            for it, data in enumerate(dataloader):
                ...
                text_feat = data["text_feat"].to(device, non_blocking=True)

                out = model(
                    obj=obj, grid=grid, seq=captions, text_feat=text_feat,
                    mode="xe")
                out = out[:, :-1].contiguous()
                captions_gt = captions[:, 1:].contiguous()
                caption_loss = loss_fn(out.view(-1, len(text_field.vocab)), captions_gt.view(-1))
                loss = caption_loss
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader), dynamic_ncols=True) as pbar:
        with torch.no_grad():
            for it, data in enumerate(dataloader):
                ...
                out, _ = model(
                    obj=obj, grid=grid, max_len=20, mode="rl",
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader), dynamic_ncols=True) as pbar:
        for it, data in enumerate(dataloader):
            ...
            if text_feat is not None:
                text_feat = text_feat.to(device, non_blocking=True)
            out, log_prob = model(
                obj=obj, grid=grid, max_len=seq_len, mode="rl",
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_spe', type=bool, default=True, help='是否使用 SPE (空间感知增强) 编码器分支')
    parser.add_argument('--use_osce', type=bool, default=True, help='是否使用 OSCE 编码器分支')
    parser.add_argument('--use_cdm', type=bool, default=True, help='是否使用 DDSP 双域稀疏')
    parser.add_argument('--spe_num_layers', type=int, default=3, help='SPE 内部层数')
    ...
    parser.add_argument('--dataset_root', type=str, default="./data/features/ctx_features_dataset") #ctx和全局特征的储存位置，二者将在同一个类中被读取。vis文件以img_id为keys;txt文件以{}_whole和{}_five为keys
    parser.add_argument('--annotations', type=str, default="./annotations")#人工标注的GT文件
    parser.add_argument('--obj_file', type=str, default="./data/features/vinvl.hdf5")#利用 Vinvl 取得的目标特征，直接以img_id作为keys
    parser.add_argument('--grid_file', type=str, default="./data/features/CLIP_features.hdf5") # 利用RSTNet101 取得的网格特征，以%d_features作为keys
    parser.add_argument('--text_feat_file', type=str, default="./data/features/retrieved_text_feats.hdf5")
    ...
    text_feat_field = TextFeatField(
        feat_file=args.text_feat_file, preload=args.preload
    )

    fields = {
        "object": object_field, "text": text_field, "img_id": RawField(),
        "grid": grid_field, "text_feat": text_feat_field
    }
    ...
    encoder = TransformerEncoder(
        ...
        use_osce=args.use_osce,
        use_cdm=args.use_cdm,
        spe_num_layers=args.spe_num_layers, 
    )
    decoder = TransformerDecoder(len(text_field.vocab), 54, 3, text_field.vocab.stoi['<pad>'])
    projector = Projector(
        f_obj=2048, f_grid=2048,
